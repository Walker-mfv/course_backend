...
export class TransactionsStatisticService {
    ...
    const pipeline = [
      ...
      ),
      {
        $group: {
          ...
          value: {
            $sum: {
              $multiply: ['$salePrice', TransactionsStatisticService.getCommissionRate(!!instructorId)],
            },
          },
        },
      },
  ...
  private getInstructorStatsPreprocessPipeline(data: ChartDataOptionsDto, project?: any, instructorId?: string): any {
    ...
    if (data['course._id_filter']) matchPipeline[0].$match['course'] = Helper.cvtObjectId(data['course._id_filter'])
