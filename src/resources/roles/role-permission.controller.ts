...
export class RolePermissionsController {
  ...
  async addPermission(@Param('roleId') roleId: string, @Body() data: AddPermissionDto) {
    const checkPermissionExist = await this.rolesService.checkPermissionExist(roleId, data.documentPermission)
  ...
  async deletePermission(@Param('roleId') roleId: string, @Param('documentPermissionId') documentPermissionId: string) {
