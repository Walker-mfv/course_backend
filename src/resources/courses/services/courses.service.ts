...
export class CoursesService extends BaseModel<Course, CourseDocument> {
  ...
  async test() {
    // const dummy = this.dummyData.find((item) => (item.basicInfo.title = 'Testing course'));
    // dummy.basicInfo.title = 'Testing course - draft';
    // dummy.basicInfo.slug = 'testing-course-draft';
    // delete dummy._id;
    // dummy.status = 'draft';
    // const item = await new this.courseModel(dummy).save();
    // return item;
