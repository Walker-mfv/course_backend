...
export default class ClientCoursesService extends CoursesService {
  ...
  async fetchClientItems(query: ClientQueryDto | IStandardizedClientQuery<Course>): Promise<CourseDocument[]> {
  ...
  async countClientItems(query: ClientQueryDto | IStandardizedClientQuery<Course>): Promise<number> {
    query = this.cvtValidClientFilterQuery(query as ClientQueryDto)
      ...
      fields.forEach((field) => {
        switch (field) {
          ...
          default:
            ...
            if (values) {
              const promise = this.countClientFieldValue(nonePaginationStages, field, values)
  ...
  private cvtValidClientFilterQuery(query: ClientQueryDto) {
    ...
    if (!!tempQuery._duration_filter) {
      ...
      const values = tempQuery._duration_filter.split(',').filter((item) => item.trim() != '')
      ...
      if (matchObj[field]) delete matchObj[field]
        ...
        for (const key in matchObj) {
          ...
          if (Array.isArray(filterArr)) {
            const fieldIdx = filterArr.findIndex((item) => Object.keys(item).indexOf(field) > -1)
            if (fieldIdx > -1) {
