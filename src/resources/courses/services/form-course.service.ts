...
export default class FormCourseService extends CoursesService {
  ...
  async deleteSection(id: string, data: DeleteCourseSectionDto): Promise<CourseDocument> {
    ...
    if (section) {
      ...
      item.details.sections = item.details.sections.filter((item) => item._id != data.sectionId)
      await item.save()
      return item
    }
  }
  ...
  async deleteUnit(id: string, data: DeleteCourseUnitDto): Promise<CourseDocument> {
    ...
    if (section) {
      ...
      if (unit) {
        ...
        item.details.sections[data.sectionIndex].units = item.details.sections[data.sectionIndex].units.filter(
          (unit) => unit._id != data.unitId
        )
        await item.save()
        return item
      }
  ...
  async deleteUnitRefDoc(unit: UnitDocument) {
    switch (unit.type) {
      case 'lecture':
        const lecture = await this.lecturesService.model.findById(unit.lecture).populate('video')
