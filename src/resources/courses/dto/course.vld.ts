...
export function IsCourseTitle({ required }: { required: boolean } = { required: false }): PropertyDecorator {
  return ValidatorComposer([IsString(), MaxLength(80)])({ required })
}
...
export function IsCourseSectionObjective({ required }: { required: boolean } = { required: false }): PropertyDecorator {
  return ValidatorComposer([IsString(), MaxLength(200)])({ required })
}

export function IsCourseSectionNumLectures(
  { required }: { required: boolean } = { required: false }
): PropertyDecorator {
  return ValidatorComposer([IsNumber(), Min(0)])({ required })
}

export function IsCourseSectionLength({ required }: { required: boolean } = { required: false }): PropertyDecorator {
  return ValidatorComposer([IsNumber()])({ required })
}

export function IsUnitType({ required }: { required: boolean } = { required: false }): PropertyDecorator {
  return ValidatorComposer([IsString(), Matches(/lecture|quiz/)])({ required })
}
...
export function IsRatingAvgScore({ required }: { required: boolean } = { required: false }): PropertyDecorator {
  return ValidatorComposer([IsNumber(), Min(0), Max(5)])({ required })
}
...
export function IsCourseCongratulationsMessage(
  { required }: { required: boolean } = { required: false }
): PropertyDecorator {
  return ValidatorComposer([IsString(), MaxLength(255)])({ required })
}
