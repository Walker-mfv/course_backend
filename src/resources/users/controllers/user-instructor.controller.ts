...
export class UserInstructorController {
  ...
  countStudents(@Req() req, @Query() query: ClientQueryDto) {
    return this.userCourseService.countStudents(this.getInstructorId(req.user, query[VIEW_INSTRUCTOR_ID]), query)
  }

  @Get('students')
  getStudents(@Req() req, @Query() query: ClientQueryDto) {
    return this.userCourseService.fetchStudents(this.getInstructorId(req.user, query[VIEW_INSTRUCTOR_ID]), query)
  }

  @Get('payment-transactions/:paymentId')
  fetchPaymentTransactions(@Param('paymentId') paymentId: string, @Query() query: ClientQueryDto) {
    return this.transactionsService.getPaymentTransactions(paymentId, query)
  }
  @Get('count-payment-transactions/:paymentId')
  countPaymentTransactions(@Param('paymentId') paymentId: string, @Query() query: ClientQueryDto) {
  ...
  fetchPendingPayments(@Req() req, @Query() query: ClientQueryDto) {
    return this.instructorPaymentsService.getPendingPayments(this.getInstructorId(req.user, query[VIEW_INSTRUCTOR_ID]))
  }
  @Get('count-payments')
  ...
  async updateReviewRespond(@Req() req, @Param('id') reviewId, @Body() data: UpdateReviewResponseDto) {
    const result = await this.courseReviewsService.updateReviewRespond(req.user._id, reviewId, data)
  ...
  private getInstructorId(user: User, viewInstructorId): string {
    const instructorId =
      viewInstructorId && this.caslAbilityFactory.createForUser(user).can('view', 'Performances')
        ? viewInstructorId
        : user._id
    return instructorId
  }
