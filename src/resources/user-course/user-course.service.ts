...
import { Archived, LearnUnit, UserCourse, UserCourseDocument } from '../user-course/schemas/user_course.schema'
...
export class UserCourseService extends BaseModel<UserCourse, UserCourseDocument> {
  ...
  async completedUnit(id: string, unitId: string) {
    ...
    if (item) {
      const existLearnUnit = item.learnDetail.learnUnits.find((item) => item.unitId == unitId)
      if (existLearnUnit) {
        ...
        item.learnDetail.progress = this.calculateProgress(item)
        await item.save()
        const lastItem = item.learnDetail.learnUnits.at(item.learnDetail.learnUnits.length - 1)
        return lastItem
      }
    }
  }
  ...
  async createUserCourse(options: { userId: string; courseId: string; salePrice?: number; date?: Date }) {
