...
export class PaymentsService extends BaseModel<Payment, PaymentDocument> {
  ...
  constructor(@InjectModel(Payment.name) protected readonly paymentModel: Model<PaymentDocument>) {
  ...
  async getPendingPayments(userId: string): Promise<Payment[] | null> {
    const pendingPayments = await this.model.find({ user: userId, status: 'pending' }).sort({ 'history.createdAt': -1 })
