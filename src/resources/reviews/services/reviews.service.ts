...
export class ReviewsService extends BaseModel<Review, ReviewDocument> {
  ...
  async updateReviewRespond(instructorId: string, reviewId: string, data: UpdateReviewResponseDto) {
  ...
  protected getAvgPreprocessPipeline(data: TotalDataOptionsDto, instructorId?: string) {
    ...
    if (data['course._id_filter']) matchPipeline[0].$match['course._id'] = Helper.cvtObjectId(data['course._id_filter'])
