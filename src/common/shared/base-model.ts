...
export abstract class BaseModel<E, D> {
  ...
  async findAll(query: ClientQueryDto | IStandardizedClientQuery<E>): Promise<D[]> {
    const pipelines = this.getPipelineStagesByClientQuery(this.cvtStandardizedQuery(query)).allStages
    return this.model.aggregate(pipelines)
  }

  async count(query: ClientQueryDto): Promise<number> {
    const pipelineStages: PipelineStage[] = [
      ...this.getPipelineStagesByClientQuery(this.standardizeClientQuery(query)).nonePaginationStages,
      {
        $count: 'n',
      },
    ]
  ...
  protected getPipelineStagesByClientQuery(
    ...
    let nonePaginationStages: PipelineStage[] = [...lookups, ...matchStage, ...sortStage, ...mergeStages]

    // search
    if (!!query.search) {
      if (query.search?._searchField == 'all' || this.processSearchFields.includes(query.search?._searchField)) {
        nonePaginationStages = [...this.preprocessSearchStages, ...nonePaginationStages]
      }
    }

    // handle exclude stages
    nonePaginationStages = nonePaginationStages.filter((item) => !exclude?.includes(Object.keys(item)[0]))
  ...
  protected standardizeClientQuery(query: ClientQueryDto): IStandardizedClientQuery<E> {
    ...
    if (!!_searchField && !!_searchValue) {
      ...
      if (_searchField == 'all') {
        ...
        if (_searchField == '_id') {
          if (Helper.isObjectId(_searchValue)) filter._id = Helper.cvtObjectId(_searchValue)
        } else {
  ...
  protected cvtStandardizedQuery(query: ClientQueryDto | IStandardizedClientQuery<E>): IStandardizedClientQuery<E> {
  ...
  protected get mergeFields(): string[] {
    return this.basicEmbedOptions.filter((item) => Helper.isPlural(item.path)).map((item) => item.path)
  }
  ...
  getLookup(data: {
    ...
    const { from, localField, foreignField = '_id', project, as = localField, lookupOne = true } = data
    let stages = []
  ...
  async validateDeletion(id: string, from: TController, foreignField: string): Promise<null | IConflictData> {
    const pipeline = [
      {
        $match: {
